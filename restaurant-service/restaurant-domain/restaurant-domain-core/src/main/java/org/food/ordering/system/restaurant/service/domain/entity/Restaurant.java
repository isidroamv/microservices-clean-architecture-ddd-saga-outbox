...
public class Restaurant extends AggregateRoot<RestaurantId> {
    ...
    public void validateOrder(List<String> failureMessages) {
        if (OrderStatus.PAID != orderDetail.getOrderStatus()) {
            failureMessages.add("Payment is not completed for order id: " + getId().getValue());
        }

        Money totalAmount = orderDetail.getProducts().stream().map(product -> {
            if (!product.isAvailable()) {
                failureMessages.add("Product is not available for order id: " + getId().getValue());
            }
            return product.getPrice().multiply(product.getQuantity());
        }).reduce(Money.ZERO, Money::add);

        if (!totalAmount.equals(orderDetail.getTotalAmount())) {
            failureMessages.add("Total amount is not correct for order id: " + getId().getValue());
        }
    }
